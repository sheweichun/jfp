import { j, render, useState, useEffect, useMemo, useCallback } from 'jfp';
...
function App() {
    ...
    const onStrangeAdd = useCallback(()=>{
    ...
    return (
      <div>
        ...
        <button onClick={onStrangeAdd}>奇怪的加号+</button>
        <div>
          数字: {count}   <br/>
          fix 数字: {fixCount} <br/>
          double 数字: {doubleCount}
        </div>
        <Random></Random>
      </div> 
    )
